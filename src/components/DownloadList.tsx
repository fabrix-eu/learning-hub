...
export function DownloadList({ resources }: { resources: Resource[] }) {
  ...
  return (
    <div className="rounded-fx border border-line bg-panel p-5">
      <p className="mb-3 text-label text-muted uppercase">Download</p>
          ...
          return (
            <li key={resource.id}>
              <a
                ...
                className="flex items-start gap-2.5 rounded-fx-sm border border-line2 bg-card px-3.5 py-3 text-small font-medium text-ink transition hover:border-violet hover:bg-violet-soft"
              >
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 block text-[11.5px] text-muted">{resourceLabel(resource.kind)}</span>
                </span>
